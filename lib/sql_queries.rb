...
"SELECT projects.title as title, (projects.funding_goal-SUM(pledges.amount)) as over_goal FROM projects JOIN pledges ON projects.id = pledges.project_id WHERE over_goal >= 0 GROUP BY projects.title, over_goal;"
end
