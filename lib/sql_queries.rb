...
"SELECT projects.title as title, (SUM(pledges.amount)-projects.funding_goal) as over_goal FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title  HAVING over_goal >= 0;"
end
